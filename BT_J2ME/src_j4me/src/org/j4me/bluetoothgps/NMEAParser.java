...
class NMEAParser {
    ...
    /**
     * Size of the string buffer. This should be a little more than the size
     * of the byte array plus 80 (the max size of an NMEA sentence).
     */
    public static final short OUTPUT_BUFFER_MAX_SIZE = 2048;

    /**
     * The maximum size of a sentence according to the NMEA standards is 82.
     * We will use 128 to be safe.
     */
     ...
     */
    private byte[] data = new byte[NMEAParser.OUTPUT_BUFFER_MAX_SIZE];
     ...
     */
    private void append(final byte[] output, final int size) {
     ...
     * 
     * @param output -
     *                the output to parse in bytes
     * @param size -
     *                the size of the output
     * @return a integer to indicate which sentences were parsed
     */
    public int parse(final byte[] output, final int size) {
        append(output, size);
        return doParse();
    }
    ...
    private int doParse() {
        ...
        while (currentIndex > 0) {
            ...
            final int sentenceStart = NMEAParser.lastIndexOf(data,
                    NMEAParser.SENTENCE_START, currentIndex);

            // Did we find the start of a sentence?
            if (sentenceStart != -1) {
                // We found the start of a sentence, look for the end.
                final int sentenceEnd = NMEAParser.indexOf(data,
                        NMEAParser.SENTENCE_END, sentenceStart, dataLength);
                ...
                if (sentenceEnd != -1) {
                    ...
                    final int sentenceTypeEnd = NMEAParser.indexOf(data,
                            NMEAParser.DELIMITER, sentenceStart, sentenceEnd);

                    // If we found the type end and the sentence end is within
                    // this sentence, then process the sentence. By checking
                    // that the
                    ...
                    if ((sentenceTypeEnd != -1)
                        ...
                        try {
                            final String type = new String(data,
                                    sentenceStart, sentenceTypeEnd
                                            - sentenceStart);

                            if ((type.equals(NMEAParser.DOLLAR_SIGN_GPRMC))
                                    && ((parsedSentenceTypes & NMEAParser.TYPE_GPRMC) == 0)) {
                                parsedSentenceTypes = parsedSentenceTypes
                                        | processSentence(data,
                                                sentenceStart, sentenceEnd,
                                                NMEAParser.TYPE_GPRMC);
                            } else if ((type
                                    .equals(NMEAParser.DOLLAR_SIGN_GPGGA))
                                    && ((parsedSentenceTypes & NMEAParser.TYPE_GPGGA) == 0)) {
                                parsedSentenceTypes = parsedSentenceTypes
                                        | processSentence(data,
                                                sentenceStart, sentenceEnd,
                                                NMEAParser.TYPE_GPGGA);
                            } else if ((type
                                    .equals(NMEAParser.DOLLAR_SIGN_GPGSA))
                                    && ((parsedSentenceTypes & NMEAParser.TYPE_GPGSA) == 0)) {
                                parsedSentenceTypes = parsedSentenceTypes
                                        | processSentence(data,
                                                sentenceStart, sentenceEnd,
                                                NMEAParser.TYPE_GPGSA);
                            }
                        } catch (final Throwable t) {
                            Log.warn("processSentence: dataLength="
                                    + dataLength + ", Start=" + sentenceStart
                                    + ", End=" + sentenceEnd, t);

                            // We are kind of screwed at this point so just
                            // return
                        ...
                        if (parsedSentenceTypes == NMEAParser.ALL_TYPES_MASK) {
                            break;
                        }
                    } else {
                    ...
                    // If this is the last sentence in the buffer, then keep
                    // the
        ...
        // If we parsed any of the sentences that we care about, put the
        // record
        // in the buffer.
     ...
     * <code>array</code> between [<code>fromIndex</code>,
     * <code>stopIndex</code>).
     * 
     * @param array
     *                is the data to scan.
     * @param b
     *                is the byte to match.
     * @param fromIndex
     *                is the first index into array to check.
     * @param stopIndex
     *                is one past the last index into array to check.
     * @return The first index where <code>b</code> was found; -1 if it was
     *         not found.
     */
    protected static int indexOf(final byte[] array, final byte b,
            final int fromIndex, final int stopIndex) {
     ...
     * <code>array</code> going backwards from <code>fromIndex</code>.
     * 
     * @param array
     *                is the data to scan.
     * @param b
     ...
     * @param fromIndex
     *                is the first index into array to check.
     * @return The last index where <code>b</code> was found; -1 if it was
     *         not found.
     */
    protected static int lastIndexOf(final byte[] array, final byte b,
            final int fromIndex) {
    ...
    /**
     * @param record
     *                is the latest location information.
     */
    private synchronized void setRecordBuffer(final GPSRecord record) {
        recordBuffer = record;
    }

    /**
     * Process the sentence of the specified type. The sentence is every ASCII
     * character stored in <code>data</code> between <code>offset</code>
     * and <code>stop</code>.
     * 
     * @param data
     *                contains the NMEA sentence to process.
     * @param offset
     *                is the index that starts the NMEA sentence within
     *                <code>data</code>.
     * @param stop
     *                is the index of the final character in the sentence.
     * @param type -
     *                the sentence type
     * @return the type of the setence processed. If the sentence cannot be
     *         processed this returns 0.
     */
    private short processSentence(final byte[] data, final int offset,
            final int stop, final short type) {
        ...
        if (stop - offset <= NMEAParser.MAX_SENTENCE_SIZE) {
            final SentenceData sentenceData = contructSentenceData(type);

            for (int i = offset; i < stop; i++) {
                final byte character = data[i];

                if (character == NMEAParser.SENTENCE_START) {
                    // Ignore the dollar sign
                } else if (character == NMEAParser.CHECKSUM_START) {
                    // First process the remaining token
                    sentenceData.processToken(tokenIndex, data, tokenStart, i
                            - tokenStart);

                    // get the sent check sum
                    try {
                        final String transmittedChecksum = new String(data,
                                i + 1, 2);
                        sentChecksum = Integer.valueOf(transmittedChecksum,
                                16).intValue();
                    ...
                    if (character == NMEAParser.DELIMITER) {
                        sentenceData.processToken(tokenIndex, data,
     ...
     */
    private SentenceData contructSentenceData(final int type) {
        switch (type) {
        case TYPE_GPRMC:
    ...
    /**
     * Interface implemented by classes that know the structure of NMEA
     * sentences.
     */
    private static interface SentenceData {
         ...
         * 
         * @param tokenIndex
         *                is the number of commas that have been seen.
         * @param data
         *                contains the NMEA sentence to process.
         * @param offset
         *                is the index that starts the NMEA sentence within
         *                <code>data</code>.
         * @param length
         *                is the number of bytes the token is.
         */
        public void processToken(short tokenIndex, byte[] data, int offset,
                int length);
    ...
    private static final class GPGSARecord implements SentenceData {
        ...
        public void processToken(final short tokenIndex, final byte[] data,
        ...
        public void applySentenceData(final GPSRecord record) {
            record.hdop = hdop;
            record.vdop = vdop;
        }
    }
