 ...
 * these values.
 * <p>
 * Instances of <code>Criteria</code> are used by the application to
 * indicate criteria for choosing the location provider in the
 * <code>LocationProvider.getInstance</code> method call. The implementation
 * considers the different criteria fields to choose the location provider
 * that best fits the defined criteria. The different criteria fields do not
 * have any defined priority order but the implementation uses some
 * implementation specific logic to choose the location provider that can
 * typically best meet the defined criteria.
 * <p>
 ...
 * specified criteria, the implementation is allowed to make its own best
 * effort selection of a location provider that is closest to the defined
 * criteria (provided that the cost criteria is met). However, an
 * implementation is not required to return a location provider if it does not
 * have any available provider that is able to meet these criteria or be
 * sufficiently close to meeting them, where the judgement of sufficiently
 * close is an implementation dependent best effort choice. It is left up to
 * the implementation to consider what is close enough to the specified
 * requirements that it is worth providing the location provider to the
 * application.
 * <p>
 * The default values for the criteria fields are specified below in the
 * table. The default values are always the least restrictive option that will
 * match all location providers. Default values:
 * <p>
 * <table border="1">
 * <tr>
 ...
 * <td>true (use the LBS on the device running this MIDlet if it meets the
 * other criteria)</td>
 * </tr>
 * <tr>
 ...
 * using the <code>set*</code> methods. It does not try to validate the
 * values of the parameters in any way. Applications may set any values it
 * likes, even negative values, but the consequence may be that no matching
...
public class Criteria {
     ...
     * location method.
     */
    private int horizontalAccuracy = Criteria.NO_REQUIREMENT;
     ...
     * location method.
     */
    private int verticalAccuracy = Criteria.NO_REQUIREMENT;

    /**
     * The desired maximum response time preference in milliseconds.
     */
    private int maxResponseTime = Criteria.NO_REQUIREMENT;

    /**
     * The preferred maximum level of power consumption. Should be one of
     * <code>NO_REQUIREMENT</code>, <code>POWER_USAGE_LOW</code>,
     * <code>POWER_USAGE_MEDIUM</code>, <code>POWER_USAGE_HIGH</code>.
     */
    private int powerConsumption = Criteria.NO_REQUIREMENT;
    ...
    /**
     * Whether the location provider should be able to determine textual
     * address information. Setting this criteria to <code>true</code>
     * implies that a location provider should be selected that is capable of
     * providing the textual address information. This does not mean that
     * every returned location instance necessarily will have all the address
     * information filled in, though.
     */
    private boolean addressInfoRequired = false;

    /**
     * When <code>true</code> the LBS on this device should be used if it
     * meets the other criteria. Local LBS gets preference over any remote LBS
     * specified by in <code>remoteDeviceAddress</code>.
     * <p>
     * To force remote LBS to be used set this value to <code>false</code>
     * and provide a remote address in <code>remoteDeviceAddress</code>.
     */
    private boolean allowLocalLBS = true;

    /**
     * The Bluetooth address of the remote GPS device to use if no local LBS
     * matches the other criteria. If this is <code>null</code> than only
     * local LBS can be used.
     */
    private String remoteDeviceAddress = null;

    /**
     * Constructs a <code>Criteria</code> object. All the fields are set to
     * the default values that are specified below in the specification of the
     ...
     * 
     * @return the power consumption level, should be one of
     *         <code>NO_REQUIREMENT</code>, <code>POWER_USAGE_LOW</code>,
     *         <code>POWER_USAGE_MEDIUM</code>,
     ...
     * 
     * @return the preferred cost setting. <code>true</code> if allowed to
     *         cost, <code>false</code> if it must be free of charge.
     ...
     *         and course. <code>true</code> means that it should be able,
     *         <code>false</code> means that this is not required.
     ...
     *         altitude. <code>true</code> means that it should be able,
     *         <code>false</code> means that this is not required.
     ...
     * 
     * @return whether the location provider should be able to normally
     *         provide textual address information. <code>true</code> means
     *         that it should be able, <code>false</code> means that this is
     *         not required.
    ...
    /**
     * Returns if the LBS on this device should be used if it meets the other
     * criteria. Local LBS gets preference over any remote LBS specified by in
     * <code>remoteDeviceAddress</code>.
     * <p>
     * If <code>false</code> only the remote LBS at
     * <code>setRemoteDeviceAddress</code> can be used.
     * 
     * @return <code>true</code> if local LBS can be used;
     *         <code>false</code> if remote LBS must be used.
     */
     ...
     * local LBS matches the other criteria. If this is <code>null</code>
     * than only local LBS can be used.
     * 
     * @return the URL of a remote LBS device. Typically this is a Bluetooth
     *         URL obtained from the
     *         <code>LocationProvider.discoverBluetoothDevices</code>
     *         method. If this is <code>null</code> than only local LBS will
     *         be used.
     ...
     * <code>NO_REQUIREMENT</code>, meaning no preference on horizontal
     * accuracy.
     * 
     * @param accuracy -
     ...
     * standard deviation for the location method. Default is NO_REQUIREMENT,
     ...
     * that typically is able to produce the location information within the
     * defined time. Default is <code>NO_REQUIREMENT</code>, meaning no
     ...
     */
    public void setPreferredResponseTime(final int time) {
     ...
     * <code>NO_REQUIREMENT</code>, meaning power consumption is not a
     * quality parameter.
     * 
     * @param level -
     *                the preferred maximum level of power consumption. Should
     *                be one of <code>NO_REQUIREMENT</code>,
     *                <code>POWER_USAGE_LOW</code>,
     *                <code>POWER_USAGE_MEDIUM</code>,
     *                <code>POWER_USAGE_HIGH</code>.
     * @see #getPreferredPowerConsumption()
     */
    public void setPreferredPowerConsumption(final int level) {
     ...
     * <p>
     * Sets whether the requests for location determination is allowed to
     * incur any financial cost to the user of the terminal.
     * <p>
     * The default is true, i.e. the method is allowed to cost.
     * <p>
     * Note that the platform implementation may not always be able to know if
     * a location method implies cost to the end user or not. If the
     ...
     * <code>LocationProvider</code> of which it is certain that using it
     * for determining the location does not cause a per usage cost to the end
     * user.
     * 
     * @param costAllowed -
     *                <code>false</code> if location determination is not
     *                allowed to cost, <code>true</code> if it is allowed to
     *                cost
     * @see #isAllowedToCost()
     */
     ...
     * and course. Default is <code>false</code>.
     * 
     * @param speedAndCourseRequired -
     *                if set to <code>true</code>, the
     *                <code>LocationProvider</code> is required to be able
     *                to normally determine the speed and course. if set the
     *                <code>false</code>, the speed and course are not
     *                required.
     * @see #isSpeedAndCourseRequired()
     */
    public void setSpeedAndCourseRequired(final boolean speedAndCourseRequired) {
        this.speedAndCourseRequired = speedAndCourseRequired;
    }

    /**
     * Sets whether the location provider should be able to determine
     * altitude.
     * <p>
     ...
     *                <code>false</code>, the altitude is not required.
     ...
     *                If set the <code>false</code>, the textual address
     *                information is not required.
     * @see #isAddressInfoRequired()
     */
    public void setAddressInfoRequired(final boolean addressInfoRequired) {
        this.addressInfoRequired = addressInfoRequired;
    }

    /**
     * Sets if the LBS on this device should be used if it meets the other
     * criteria. Local LBS gets preference over any remote LBS specified by in
     * <code>remoteDeviceAddress</code>.
     * <p>
     * To force remote LBS to be used set this value to <code>false</code>
     * and provide a remote address to <code>setRemoteDeviceAddress</code>.
     * <p>
     * Default is <code>true</code>.
     * 
     * @param allowLocalLBS
     *                when <code>true</code> uses LBS on the current device
     *                if it meets the other criteria; when <code>false</code>
     *                forces remote LBS to be used.
     */
    public void setAllowLocalLBS(final boolean allowLocalLBS) {
        this.allowLocalLBS = allowLocalLBS;
    }

    /**
     * Sets the Bluetooth address of the remote GPS device. This address can
     * be obtained from the
     * <code>LocationProvider.discoverBluetoothDevices</code> method. Once
     * the address is known, it should be stored using the Record Management
     * System (RMS) for the next time the MIDlet is run. If this value is set
     * to <code>null</code>, the default, than only LBS providers built
     * into the device running the MIDlet will be used.
     * 
     * @param address -
     ...
     */
    public void setRemoteDeviceAddress(final String address) {
