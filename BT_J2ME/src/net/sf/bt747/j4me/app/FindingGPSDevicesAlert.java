 ...
 * discovery to create a list of nearby devices. It disappears to give them a
 * selection once it is done. This usually takes a few seconds, but sometimes as
 * many as 30.
 */
public class FindingGPSDevicesAlert extends ProgressAlert {
     ...
     * starts, then if the user goes back and screen and comes back to this one,
     * a second thread will start. This <code>bluetoothLock</code> prevents
     * the second thread from doing work until the first completes. This is not
     * the most efficient way of doing things because the first thread's results
     * will be discarded, but it is easy logic and only requires the user wait a
     * few more seconds and should almost never happen.
     * <p>
