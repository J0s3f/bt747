 ...
 * spaces do not appear. There are several special markup strings for
 * controling spacing:
 * <ul>
 * <li><code>\n</code> - Goes to a newline and adds a 1/2 space separator
 * <li><code>\r</code> - Goes to a newline, but does not add a separator
 * </ul>
 * The string is displayed using the theme's font and color unless overridden
 * by the <code>setFont</code> and <code>setFontColor</code> methods.
 * <p>
 ...
 * <code>paint</code>, <code>getWidth</code>, and <code>getHeight</code>.
 * A good way to generate font images is with the <a href="-
 * http://www.angelcode.com/products/bmfont/"> AngelCode Bitmap Font Generator</a>.
 */
public class Label extends Component {
    ...
    /**
     * Each element is a substring of <code>text</code> that appears on a
     * single line.
     */
    ...
    /**
     * @param text
     *                is the string to display. If <code>null</code> the
     *                label will be hidden.
     */
    public synchronized void setLabel(final String text) {
        this.text = text;
        invalidate();
    }
     ...
     * <code>getFont()</code> because if the user has not overriden the
     * font, the theme's font will be returned.
     * 
     * @param theme
     *                is the application's theme.
     * @return The <code>Font</code> used by the label.
     */
    private Font getFont(final Theme theme) {
        if (font != null) {
            return font;
        } else {
    ...
    /**
     * @param font
     *                is the font to use for this label. If <code>null</code>
     ...
     */
    private int getFontColor(final Theme theme) {
        if (fontColor >= 0) {
            return fontColor;
        } else {
    ...
    /**
     * @param color
     *                is the font color used to display the label. If negative
     *                the theme's font will be used.
     * 
     * @see Theme#getFontColor()
     */
    ...
    protected synchronized void paintComponent(final Graphics g,
            final Theme theme, final int width, final int height,
            final boolean selected) {
        if (text != null) {
            // Set the font on the graphics object.
            final Font font = getFont(theme);
            g.setFont(font);

            final int fontColor = getFontColor(theme);
            g.setColor(fontColor);

            final int fontHeight = font.getHeight();
            final int paragraphSpacing = fontHeight / 2;
            ...
            if ((lines == null) || (widthForLines != width)) {
                lines = Label.breakIntoLines(font, text, width);
                widthForLines = width;
            }

            // Get the top and bottom of current graphics clip.
            final int clipTop = g.getClipY();
            final int clipBottom = clipTop + g.getClipHeight();
    ...
    /**
     * Returns the dimensions of the label for the current string. The width
     * is equal to the width of the longest line of text. The height is equal
     * to the sum of all the lines including line breaks between paragraphs.
     * 
    ...
    protected synchronized int[] getPreferredComponentSize(final Theme theme,
        ...
        if (text != null) {
            ...
            if ((lines == null) || (widthForLines != viewportWidth)) {
                lines = Label.breakIntoLines(font, text, viewportWidth);
            ...
            for (int i = 0; i < lines.length; i++) {
                if (lines[i] == null) {
                    ...
                    final int lineWidth = font.stringWidth(lines[i]);
    ...
    /**
     * Breaks <code>string</code> apart at line breaks. The returned array
     * has strings that can be written using <code>font</code> in fewer than
     * <code>width</code> pixels.
     * <p>
     * The <code>string</code> will be split apart at non-alphanumeric
     * characters. However, if a string is too long and cannot be broken
     * nicely, it will be split at the last alphanumeric character that fits
     * the line <code>width</code>.
     * <p>
     * Newline characters (<code>'\n'</code>) are automatically made into
     * line breaks. An additional empty line, represented by a
     * <code>null</code> string in the returned array, is added. It can then
     * be painted as a space to indicate a new paragraph.
     * 
     * @param font
     *                is the <code>Font</code> used to write
     *                <code>string</code>.
     * @param string
     ...
     *         breaks.
     */
    public static String[] breakIntoLines(final Font font,
            final String string, int width) {
        String[] lines = null;

        if (string != null) {
            // A container for holding lines as we parse them.
            final Vector parsedLines = new Vector();
            String newLine;
            ...
            final int charWidth = font.charWidth('O');
            width -= charWidth;
            ...
            for (int i = 0; i < chars.length; i++) {
                final boolean isSeparator = (chars[i] == '-')
                        || (chars[i] == '/');
                final boolean isWhiteSpace = (chars[i] == ' ');

                final boolean isNewLine = (chars[i] == '\n');
                final boolean isCarrageReturn = (chars[i] == '\r');

                final boolean isLineBreak = isNewLine || isCarrageReturn;
                final int lineWidth = font.charsWidth(chars, lineStart, i
                        - lineStart);
            ...
            final Enumeration e = parsedLines.elements();
            int i = 0;
