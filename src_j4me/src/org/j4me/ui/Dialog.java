 ...
 * components. If you need direct control over the appearance of the screen
 * use the <code>DeviceScreen</code> class.
 * 
...
public abstract class Dialog extends DeviceScreen {
    ...
    /**
     * The offset into the form that has been scrolled down. 0 indicates the
     * very top of the form is shown at the top of the screen. 100 would
     * indicate that the current top of the screen (i.e. y == 0) shows the
     * 100th pixel down in the form. If the first component were 80 pixels
     * tall and the second 40, 100 would mean the bottom half of the second
     * component would be at the top of the screen.
     */
    ...
    /**
     * The number of pixels high the entire form is with all the components on
     * it. If this number is greater than the height of the screen, a vertical
     * scrollbar will be added to the side.
     * <p>
     * When the form is painted for the first time this number will be
     * calculated. If the components on the form are changed, this will be
     * reset back to <code>null</code>.
    ...
    /**
     * A flag indicating if the screens layout is no longer valid. It will be
     * calculated on the next <code>paint</code>.
     * <p>
    ...
    public Dialog() {
        ...
        final String ok = theme.getMenuTextForOK();
     ...
     * <p>
     * Notifies all the components they are hidden. Classes that override this
     * method should be sure to call <code>super.onDeselection</code>.
     * 
    ...
    public void hideNotify() {
        ...
        while (e.hasMoreElements()) {
            final Component c = (Component) e.nextElement();
            c.visible(false);
        }
     ...
     * <code>index</code>. The previous component at that position is
     * discarded.
     * 
     * @param component
    ...
    /**
     * Returns the index of the currently selected component. It is the one
     * the user can currently enter data into.
     * 
    ...
    /**
     * Returns the index of the component that contains the given pixel. If no
     * component is at that location, for example it is in the spacing between
     * components, then <code>-1</code> is returned.
     * <p>
     * This method assumes that the click is relative to the form area. It may
    ...
    private synchronized int getAt(final int x, final int y) {
        ...
        for (int i = 0; i < absoluteHeights.length - 1; i++) {
            ...
            final int top = absoluteHeights[i];
            final int topOfNext = absoluteHeights[i + 1];
            final int bottom = topOfNext - spacing;

            final int left = margin;
            final int right = left + componentWidths[i];

            // Does the point fall within this component?
            if ((absY >= top) && (absY < bottom) && (x >= left)
                    && (x < right)) {
    ...
    public synchronized void setSelected(final int index) {
        ...
        final int maxScroll = absoluteHeights[absoluteHeights.length - 1]
                - getHeight();
    ...
    protected synchronized void paint(final Graphics g) {
        ...
        final int formWidth = getWidth();
        final int bottomOfScreen = topOfScreen + height;

        // Walk the list of components and paint them.
        final Enumeration list = components.elements();

        for (int i = 0; i < absoluteHeights.length - 1; i++) {
            final Component c = (Component) list.nextElement();
            final int componentTop = absoluteHeights[i];
            final int componentBottom = absoluteHeights[i + 1] - spacing;
            ...
            } else // visible
            {
                ...
                final int componentX = margin;
                final int componentY = componentTop - topOfScreen;
                final int componentWidth = formWidth;
                final int componentHeight = componentBottom - componentTop;

                // Paint this component.
                if (DeviceScreen.intersects(g, componentX, componentY,
                        componentWidth, componentHeight)) {
                    final boolean selected = (i == highlightedComponent);

                    c.paint(g, theme, this, componentX, componentY,
                            componentWidth, componentHeight, selected);
                }
            }
        }
    }
    ...
    protected void paintVerticalScrollbar(final Graphics g, final int x,
        ...
        UIManager.getTheme().paintVerticalScrollbar(g, x, y, width, height,
                offset, formHeight);
    }

    /**
     * Returns the margin for the left and right of the screen. This also pads
     * the top and bottom of the form; however, scrolling can make the text
     * appear at the very top and bottom of the screen.
     * 
     * @return The number of pixels between components and the edges of the
     *         screen.
     */
    ...
    /**
     * Sets the margin for the left and right of the screen. This also pads
     * the top and bottom of the form; however, scrolling can make the text
     * appear at the very top and bottom of the screen.
     * 
     * @param margin
     ...
     * portion excludes the vertical scrollbar on the right of the screen (if
     * it exists).
     * 
    ...
    public int getWidth() {
        ...
        if (hasVerticalScrollbar()) {
            final int scrollbarWidth = UIManager.getTheme()
     ...
     * form.
     * 
     * @return <code>true</code> if the form has a vertical scrollbar and
    ...
    /**
     * Lays out the components on the screen. This also keeps the scroll
     ...
     * positioning. The form's overall height, the sum of all the components,
    ...
    private synchronized void calculateLayout(final Theme theme,
        ...
        for (int i = 0; list.hasMoreElements(); i++) {
            ...
            final int[] dimensions = c.getPreferredSize(theme, width, height);
     ...
     * screen does not scroll too far.
     * </ol>
     * 
     * @param down
    ...
    private int nextHighlightableComponent(final boolean down,
        ...
        if (down) {
            ...
            // Walk through the components until we find the next one to
            // highlight.
            final int components = size();

            for (int next = highlightedComponent + 1; next < components; next++) {
                // Otherwise have we walked off the end of the screen?
                final int nextBottom = absoluteHeights[next + 1];

                if (nextBottom > maxBottom) {
                    if (next - 1 == highlightedComponent) {
                        // Move at least to the next component, even if it
                        // cannot
                        // be completely shown because it is bigger than the
                        // screen.
                        return next;
                    } else {
        ...
        } else // up
        {
            if (highlightedComponent > 0) {
                ...
                final int maxTop = topOfScreen - maxScroll;

                // Walk through the components until we find the next one to
                // highlight.
                for (int next = highlightedComponent - 1; next >= 0; next--) {
                    // Otherwise have we walked off the end of the screen?
                    final int nextTop = absoluteHeights[next];
                    ...
                    if (nextTop < maxTop) {
                        if (next + 1 == highlightedComponent) {
                            ...
                            // Show the one before next so it can be
                            // completely
                            // shown on the screen.
                            return next + 1;
                        }
    ...
    /**
     * Moves the viewport of the form up or down and calls
     * <code>repaint</code>.
     * <p>
     * The amount scrolled is dependent on the components shown. Typically one
     * scroll increment advances to the next component. However, if components
    ...
    private void scroll(final boolean down) {
        ...
        final int topOfForm = 0;
        final int screenHeight = getHeight();
        final int bottomOfForm = absoluteHeights[absoluteHeights.length - 1]
                - screenHeight;
        final int bottomOfScreen = topOfScreen + screenHeight;
        ...
        final int max = screenHeight * 9 / 10;

        // Get the next component that can be highlighted.
        final int current = highlightedComponent;
        ...
        if (hasVerticalScrollbar()) {
            ...
            // Calculate how far to scroll to get to the next highlighted
            // component.
            if (down) {
                final int currentBottom = absoluteHeights[current + 1];

                if (currentBottom > bottomOfScreen) {
                    // The current component actually is clipped by the bottom
                    // of
                    // the screen (because it is too big to show all at once).
                    // Just scroll down we can see more of it.
                    scroll = max;
                } else {
                    // Get the screen position of the next highlightable
                    // component.
                    final int nextTop = absoluteHeights[highlightedComponent];
                    final int nextBottom = absoluteHeights[highlightedComponent + 1];

                    if ((nextTop > topOfScreen)
                            && (nextBottom < bottomOfScreen)) {
            ...
            {
                final int currentTop = absoluteHeights[current];
                ...
                if (currentTop < topOfScreen) {
                    ...
                    if (highlightedComponent == 0) {
                        ...
                        // Get the bottom of the component above the next
                        // highlighted one.
                        final int previousBottom = absoluteHeights[highlightedComponent]
                                - spacing;
    ...
    private void keyPressedOrRepeated(final int keyCode) {
    ...
    protected synchronized void pointerPressed(final int x, final int y) {
        ...
        if (hasVerticalScrollbar()) {
            ...
            final int scrollbarWidth = UIManager.getTheme()
                    .getVerticalScrollbarWidth();
            final int scrollbarX = screenWidth - scrollbarWidth;
            ...
            if (x >= scrollbarX) {
                ...
                final int height = getHeight();
                final int formHeight = absoluteHeights[absoluteHeights.length - 1];

                final int scrollableHeight = formHeight - height;
                final double trackbarPercentage = (double) height
                        / (double) formHeight;
                int trackbarHeight = (int) MathFunc.round(height
                        * trackbarPercentage);
                ...
                final double offsetPercentage = (double) topOfScreen
                        / (double) scrollableHeight;
                final int center = rangeStart
        ...
        if (movedScrollbar == false) {
            ...
            if (c != null) {
                ...
                final int px = x - c.getX();
                final int py = y - c.getY();
