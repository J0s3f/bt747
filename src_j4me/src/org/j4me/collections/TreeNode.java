...
/**
 * <code>TreeNode</code> objects can be combined into a tree. The tree is
 * not a special tree such as a balanced tree. It can have any number of
 * levels and each node can have any number of children.
 * <p>
 ...
 * node's parent and children. A node's tree is the set of all nodes that can
 * be reached by starting at the node and following all the possible links to
 ...
 * <p>
 * Every <code>TreeNode</code> can hold a reference to one user object. It
 * is up to the developer to decide what the object is and how to use it. For
...
public class TreeNode {
    /**
     * This node's parent node. If this is the root of the tree then the
     * parent will be <code>null</code>.
     */
    private TreeNode parent;

    /**
     * An array of all this node's child nodes. The array will always exist
     * (i.e. never <code>null</code>) and be of length zero if this is a
     * leaf node.
     * <p>
     ...
     * copying and management. However to get the array it can just be
     * returned instead of creating a new array from the <code>Vector</code>
     * each time. The tree is used frequently enough for reading course
     * elements that this difference makes a small impact on rendering.
     */
     ...
     * 
     * @param userObject
     *                is an object this node encapsulates. It is up to the
     *                developer to maintain its type. To get the object back
     *                out call <code>getUserObject</code>.
     */
     ...
     * 
     * @param child
     *                is the node to add to the tree as a child of
     *                <code>this</code>
     * 
     * @param index
     *                is the position within the children list to add the
     *                child. It must be between 0 (the first child) and the
     *                total number of current children (the last child). If it
     *                is negative the child will become the last child.
     */
    public void add(final TreeNode child, final int index) {
    ...
    /**
     * Adds the <code>child</code> node to this container making this its
     * parent. The child is appended to the list of children as the last
     * child.
     */
     ...
     * 
     * @param index
     *                is the position of the child. It should be between 0
     *                (the first child) and the total number of children minus
     *                1 (the last child).
    ...
    public TreeNode remove(final int index) {
        ...
        final TreeNode node = children[index];
        node.parent = null;
     ...
     *         <code>false</code> if it does not have any children.
     */
    ...
    public int index() {
        if (parent != null) {
            for (int i = 0;; i++) {
                final Object node = parent.children[i];
    ...
    /**
     * Gets this node's depth in the tree. The root node will have a depth of
     * 0, first-level nodes will have a depth of 1, and so on.
     * 
     * @return The depth of this node in the tree.
     */
    public int depth() {
        final int depth = recurseDepth(parent, 0);
        return depth;
    }
     ...
     * 
     * @param node
     *                is the node to recursively check for its depth. This
     *                should be set to <code>parent</code> by the caller.
     * @param depth
     *                is the depth of the current node (i.e. the child of
     *                <code>node</code>). This should be set to 0 by the
     *                caller.
     */
    private int recurseDepth(final TreeNode node, final int depth) {
     ...
     * This will be <code>null</code> when the user has not assigned any
     * data to this node.
     */
     ...
     * 
     * @param userObject
     *                is the programmer defined object to attach to this node
     *                in the tree. Set it to <code>null</code> to clear any
     *                objects.
     */
    ...
    /**
     * Gets the user defined object attached to this node. It must be cast
     * back to what it was inserted as. It is up to the developer to make this
     * cast.
     * 
     * @return The programmer defined object attached to this node in the
     *         tree. Returns <code>null</code> if no object is attached.
     */
