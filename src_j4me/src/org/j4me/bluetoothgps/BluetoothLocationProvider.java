...
class BluetoothLocationProvider extends LocationProvider {
    ...
    public static LocationProvider getInstance(final Criteria criteria)
        ...
        if (BluetoothLocationProvider.instance != null) {
            throw new LocationException(
                    "Bluetooth GPS socket already in use.");
        }

        // See if we meet the criteria.
        if (BluetoothLocationProvider.matchesCriteria(criteria)) {
            final String url = criteria.getRemoteDeviceAddress();
     ...
     *                 prohibited.
     */
    private BluetoothLocationProvider(final String remoteDeviceBTAddress,
     ...
     * possible without first connecting to it. We assume it is a NMEA with
     * SBAS (Satellite Based Augmentation System) such as WAAS. These devices
     ...
     * <li>Response times of a second. In actuality these devices work at
     * 4800 baud or more meaning they send at least 8 sentences a second (480
     * characters / 80 characters sentence). Not all of these sentences give
     * us the necessary data, but usually it comes in a minimum of twice a
     * second.
     ...
     * <li>Address information is not supplied. Only the WGS 84 coordinates
     * are obtained.
     * <li>A remote URL, typically to a Bluetooth GPS device, must be
     * supplied. Otherwise only local LBS found on the device running this
     * MIDlet are allowed.
     * </ul>
     * 
     * @param criteria
     *                defines the applications LBS requirements.
     * @return <code>true</code> if the Bluetooth GPS meets the specified
     *         criteria; <code>false</code> if it does not.
     */
    public static boolean matchesCriteria(final Criteria criteria) {
        ...
        if (criteria.isAltitudeRequired()
                && (criteria.getVerticalAccuracy() < 1)) // in meters
        {
            return false;
        }

        if ((criteria.getPreferredResponseTime() > 500)
                && (criteria.getPreferredResponseTime() < 500)) // in
                                                                // milliseconds
        {
            return false;
        }
     ...
     * 
     * @param bturl -
     *                The url to the bluetooth device
     * @throws ConnectionNotFoundException -
     *                 If the target of the name cannot be found, or if the
     *                 requested protocol type is not supported.
     * @throws IOException -
     ...
     *                 prohibited.
     */
    public static BluetoothLocationProvider getInstance(final String bturl)
    ...
    public Location getLocation(int timeout) throws LocationException,
        ...
        do {
            if ((state == LocationProvider.OUT_OF_SERVICE) || (gps == null)) {
                throw new LocationException(
                        "Bluetooth location provider is out of service");
            }
            ...
            if (loc == null) {
                ...
                if (System.currentTimeMillis() - start > timeout) {
                    throw new LocationException(
                            "Timed out getting location from Bluetooth location provider");
                }
            }
        } while (loc == null);
    ...
    public void reset() throws IOException {
        if (gps != null) {
            // Perform a warm-start on the GPS receiver so that it re-acquires
            // a fix.
            gps.reacquireFix();

            // Mark the provider as unavailable.
            gps.setProviderState(LocationProvider.TEMPORARILY_UNAVAILABLE);
        }
    }
    ...
    public void close() {
        if (gps != null) {
            ...
            state = LocationProvider.OUT_OF_SERVICE;
