...
public abstract class AbstractTileFactory extends TileFactory {
	...
	private static final Logger LOG = Logger.getLogger(DefaultTileFactory.class
			.getName());
	...
	private static class TileDownloader implements Runnable {
				...
				if (urlString.startsWith("virtual-")) {
					try {
						...
						if (img == null) // download
						{
							...
							Tile parentTile = new Tile("osm", parentKeys[1],
									parentKeys[2], parentKeys[0], baseURL + "/"
											+ parentKeys[0] + "/"
											+ parentKeys[1] + "/"
											+ parentKeys[2] + ".png",
									parentKeys[0] + "/" + parentKeys[1] + "/"
											+ parentKeys[2]);
							downloadTile(baseURL + "/"
									+ (mapZoom - parentKeys[0]) + "/"
									+ parentKeys[1] + "/" + parentKeys[2]
									+ ".png", bout, buf, parentTile);
							img = cache.get(xyIndexParent);
							// System.out.println("****************************");
						}
						HashMap<String, BufferedImage> calculatedTilesLowerZoomHashMap = computeScaledImage(
								tile, xyIndexParent, img);
						if (calculatedTilesLowerZoomHashMap != null) {
							Set<String> keySet = calculatedTilesLowerZoomHashMap
									.keySet();
							for (String tileKey : keySet) {
								BufferedImage scaledImage = calculatedTilesLowerZoomHashMap
										.get(tileKey);
								ByteArrayOutputStream baos = new ByteArrayOutputStream();
								ImageIO.write(scaledImage, "png", baos);
								byte[] bytesOut = baos.toByteArray();
								Integer[] parentKeys = getParentKeys(tileKey);
								Tile t = new Tile("osm", parentKeys[1],
										parentKeys[2], parentKeys[0], baseURL
												+ "/" + parentKeys[0] + "/"
												+ parentKeys[1] + "/"
												+ parentKeys[2] + ".png",
								...
								cache.put(getCacheTileKey(t), bytesOut,
										scaledImage);
								t.setNeedsToBeDownLoaded(false);
							}
						}
						continue tileDownloadLoop;
						// @TODO: create lower zoom tiles and place it into the
						// cache
					} catch (IOException ex) {
						Logger.getLogger(AbstractTileFactory.class.getName())
		...
		private boolean downloadTile(String urlString,
				ByteArrayOutputStream bout, byte[] buf, Tile tile) {
			try {
				...
				/*
				 * setting these timeouts ensures the read does not lock up
				 * indefinitely when the server has problems.
				 */
				...
				BufferedImage img = PaintUtils
						.loadCompatibleImage(new ByteArrayInputStream(bimg));
				cache.put(getCacheTileKey(tile), bimg, img);
				tile.setImage(img);
				return true;
			} catch (Exception e) {
				if (e instanceof SocketTimeoutException) {
					LOG.log(Level.INFO, name + " timed out downloading tile: "
					...
					LOG.log(Level.SEVERE, name + " error downloading tile: "
							+ tile.getKey() + " url: " + urlString, e);
				}
			} catch (OutOfMemoryError e) {
				LOG.log(Level.SEVERE, name + " out of memory " + tile.getKey()
						+ " url: " + urlString, e);
			}
			return false;
		}
	...
	private static HashMap<String, BufferedImage> computeScaledImage(
			Tile requestedTile, String keyParentTile,
		...
		int type = (unscaledImage.getTransparency() == Transparency.OPAQUE) ? BufferedImage.TYPE_INT_RGB
				: BufferedImage.TYPE_INT_ARGB;
		...
		AffineTransform at = AffineTransform.getScaleInstance(
				(double) targetWidth / bsrc.getWidth(), (double) targetHeight
						/ bsrc.getHeight());
		g.drawRenderedImage(bsrc, at);
		try {
			ImageIO.write(bdest, "png", new File(
					"/Users/selfemp/Desktop/scaledimg.png"));
		} catch (IOException ex) {
			Logger.getLogger(AbstractTileFactory.class.getName()).log(
					Level.SEVERE, null, ex);
		}
		return getTileImages(keyParentTile, bdest, requestedTile);
	}

	private static HashMap<String, BufferedImage> getTileImages(String key,
			BufferedImage tileImage, Tile requestedTile) {
		...
		if (isXEven == true && isYEven == true) {
			...
			ret.put(tileKey, tileImage.getSubimage(256, 256, 256, 256));
			try {
				ImageIO.write(tileImage.getSubimage(0, 0, 256, 256), "png",
						new File("/Users/selfemp/Desktop/scaledimg-t1.png"));
				ImageIO.write(tileImage.getSubimage(0, 256, 256, 256), "png",
						new File("/Users/selfemp/Desktop/scaledimg-t2.png"));
				ImageIO.write(tileImage.getSubimage(256, 0, 256, 256), "png",
						new File("/Users/selfemp/Desktop/scaledimg-t3.png"));
				ImageIO.write(tileImage.getSubimage(256, 256, 256, 256), "png",
						new File("/Users/selfemp/Desktop/scaledimg-t4.png"));
			} catch (IOException ex) {
				Logger.getLogger(AbstractTileFactory.class.getName()).log(
						Level.SEVERE, null, ex);
			}
	...
	private static BufferedImage tryTileInCache(Tile tile) {
		...
		if ((image == null)
				|| (imageDate == null)
				|| ((System.currentTimeMillis() - imageDate) > 7 * 24 * 60 * 60
						* 1000)) {
			tile.setNeedsToBeDownLoaded(true);
			tileHandler.tileStatusChanged();
		}
	...
	private static class TilesHandler {
		 ...
		 * the tilePoint is (3,5), then the appropriate tile will be located and
		 * returned.
		 * 
		 * @param tilePoint
		 * @param zoom
