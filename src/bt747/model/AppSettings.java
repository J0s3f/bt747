...
public class AppSettings implements BT747Thread {
	...
	/**
	 * The GPS time offset (UTC vs. local time). (used in adjusting the time in
	 * the output formats).
	 * 
	 * @deprecated
	 */
	...
	/**
	 * Parameter index to get the Google Map key. This is not a stored setting.
	 * 
	...
	/**
	 * Indicates if this is the first time a connection is made in the app. Used
	 * to set some device dependent settings needed when no connection.
	 */
	 ...
	 * Replaces {@link #GPSTIMEOFFSETHOURS_OBSOLETE} (used in adjusting the time
	...
	/**
	 * When true, creates missing fields in the source during the conversion
	 * process.
	 */
	...
	/*
	 * True if waypoint information (comment) should be written
	 */
	...
	private void updateSettings(final int versionX100) {
		...
		case 52:
			...
			/* fall through */
		case 53:
			setBooleanOpt(IS_WRITE_WAYPOINT_COMMENT, true);
			/* fall through */
		case 54:
			if (versionX100 == 0) {
	...
	public final int getDownloadTimeOut() {
		int DownloadTimeOut = getLocalIntOpt(AppSettings.C_DOWNLOADTIMEOUT_IDX,
	...
	/**
	 * Mac systems exhibit an issue and this parameters permits the application
	 * to detect that it has to be bypassed.
	 * 
	 * @param arg
	...
	private final float getLocalFloatOpt(final int idx, final int size) {
		return JavaLibBridge.toFloatBitwise(Conv
				.hex2Int(getStringOpt(idx, size)));
	}
	...
	/**
	 * List of actions regarding listeners. Protected with listenerActionsSema.
	 */
	...
	/**
	 * Get a path. Introduced so that an override is possible on some systems to
	 * take additional parameters into account like the card number.
	 * 
	 * @param pathType
	...
	public final void removeListener(final ModelListener l) {
		listenerActionsSema.down();
		listenerActions
				.addElement(new ListenerAction(l, ListenerAction.REMOVE));
		hasListenerChangeAction = true;
		listenerActionsSema.up();
	}
	...
	// Type, idx, start, size
	{
			...
			{ AppSettings.INT, AppSettings.FILETIMEOFFSET,
					...
					AppSettings.C_FILETIMEOFFSET_SIZE },
			{ AppSettings.INT, AppSettings.PORTNBR, AppSettings.C_PORTNBR_IDX,
					AppSettings.C_PORTNBR_SIZE },
			...
			{ AppSettings.INT, AppSettings.TAG_MAXTIMEDIFFERENCE,
					...
					AppSettings.C_TAG_MAXTIMEDIFFERENCE_SIZE },
			{ AppSettings.INT, AppSettings.GPSTYPE, AppSettings.C_GPSTYPE_IDX,
					AppSettings.C_GPSTYPE_SIZE },
			...
			{ AppSettings.BOOL, AppSettings.ADVFILTACTIVE,
					...
					AppSettings.C_ADVFILTACTIVE_SIZE },
			{ AppSettings.BOOL, AppSettings.GPXUTC0, AppSettings.C_GPXUTC0_IDX,
					AppSettings.C_GPXUTC0_SIZE },
			...
			{ AppSettings.INT, AppSettings.MIN_NSAT, AppSettings.C_minNSAT_IDX,
					AppSettings.C_minNSAT_SIZE },
			...
			{ AppSettings.BOOL, AppSettings.CREATE_MISSING_FIELDS,
					AppSettings.CREATE_MISSING_FIELDS_IDX,
					AppSettings.CREATE_MISSING_FIELDS_SIZE },
			{ AppSettings.BOOL, AppSettings.AUTOCENTERMAP,
					AppSettings.AUTOCENTERMAP_IDX,
					AppSettings.AUTOCENTERMAP_SIZE },
