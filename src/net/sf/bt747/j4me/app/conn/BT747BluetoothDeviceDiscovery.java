...
/**
 * Class taken from J4ME to be modified.<br>
 * Goal is to make discovered devices appear in GUI as they are discovered.<br>
 * The user can then select a device before discovery is complete.<br>
...
class BT747BluetoothDeviceDiscovery implements DiscoveryListener {
    ...
    /**
     * A list of discovered devices. This is a list of {@link RemoteDevice}
     * objects.
     */
     ...
     * positive number and have one of the following values.
     * <p>
     ...
     *         <code>null</code>.
     * @throws IOException
     *             if any Bluetooth I/O errors occur. For example if another
     *             Bluetooth discovery operation is already in progess.
    ...
    public String[][] discoverNearbyDeviceNamesAndAddresses()
        ...
        // Did it fail?
        if (getDeviceDiscoveryResult() != DiscoveryListener.INQUIRY_COMPLETED) {
            throw new IOException(
                    "Search for Bluetooth devices failed (code "
                            + getDeviceDiscoveryResult() + ").");
        }

        // Create a list of the discovered devices.
        final Vector discoveredDevices = getDiscoveredDevices();
        final Enumeration devices = discoveredDevices.elements();
        final String[][] ret = new String[discoveredDevices.size()][2];

        for (int i = 0; devices.hasMoreElements(); i++) {
            final RemoteDevice device = (RemoteDevice) devices.nextElement();
            ...
            final String[] val = new String[] { name, address };
            ret[i] = val;
        }
     ...
     * 
     * @param remote
     *            the remote device to inquire services for
     * @throws BluetoothStateException
     */
    public void doDiscoverService(final RemoteDevice remote)
            throws BluetoothStateException {
        //
        ...
        //
        // note: you don't have to retrieve all attribute everytime. in fact,
        // you
        // can put null for attr and retrieve just the default values. that
        // probably
        // good enough for most cases. we list all attributes for demo purpose
        //
        // note: For Nokia 6230, there is a limit (13) of number of attribute
        // to retrieve
        // and this large array will cause error. If you are using Nokia 6230,
        // you should
        // reduce this array to 13 elements. Just pick any 13 of them and make
        // sure you pick
         ...
         * 0x000D, 0x0100, 0x0101, 0x0102, 0x0200, 0x0201, 0x0301, 0x0302,
         * 0x0303, 0x0304, 0x0305, 0x0306, 0x0307, 0x0308, 0x0309, 0x030A,
         * 0x030B, 0x030C, 0x030D, 0x030E, 0x0310, 0x0311, 0x0312, 0x0313 };
         */
        final int[] attr = new int[] { 0x0000, 0x0001, 0x0002, 0x0003,
        ...
        discoveryAgent.searchServices(attr, // attributes to retrieve from
                // remote device
                new UUID[] { new UUID(0x0100) }, // search criteria, 0x0100
                // = L2CAP
                remote, this); // direct discovery response to Listener
        // object
    }
     ...
     */
    public void servicesDiscovered(final int arg0,
     ...
     */
    public void inquiryCompleted(final int inquiryResult) {
        deviceDiscoveryResult = inquiryResult;
    }

    /**
     * Get the list of discovered devices. This is a list of
     * {@link RemoteDevice} objects.
     * 
     ...
     *         <code>null</code>
     */
    public RemoteDevice getDeviceByFriendlyName(final String friendlyName)
            throws IOException {
        final Enumeration devices = discoveredDevices.elements();
    ...
    /**
     * Get the first discovered service from selected remote device. Your
     * application call this method after your app receives COMPLETED callback
     * event. This will return the first service that match your UUIDs in
     * startInquiry().
     * 
     ...
     * 
     * @return {@link #SERVICE_SEARCH_IN_PROGRESS} if search is still in
     *         progress.
     */
     ...
     * 
     * @param serviceRecord
     *            - A service record from the bluetooth device
     * @return the channel id. If the id could not be determined,
     *         <code>null</code> is returned.
     */
    public static String findChannelId(final ServiceRecord serviceRecord) {
        // e1 is ProtocolDescriptorList
        final DataElement e1 = serviceRecord.getAttributeValue(0x0004);

        for (final Enumeration enum1 = (Enumeration) e1.getValue(); enum1
                .hasMoreElements();) {
            // e2 is one ProtocolDescriptor
            final DataElement e2 = (DataElement) enum1.nextElement();
            final Enumeration enum2 = (Enumeration) e2.getValue();

            // e3 is RFCOMM (0x0003)
            final DataElement e3 = (DataElement) enum2.nextElement();

            if (e3.getValue().equals(new UUID(0x0003))) {
                // e4 is channel id
                final DataElement e4 = (DataElement) enum2.nextElement();
                final int id = (int) e4.getLong();
