 ...
 * provides methods to iterate over it's LogPackets. <br>
...
public class LogBlockChunk implements Iterable<LogPacket> {
    ...
    static class ChunkIterator implements Iterator<LogPacket> {
        ...
        ChunkIterator(final DataInputStream aDIS, final LogDataParser aParser,
                final LogFormat aFormat, int aMinLogPacketSize) {
            // System.out.println("creating chunk iterator"); //$NON-NLS-1$

            this.dis = aDIS;
            this.parser = aParser;
            this.format = new LogFormat(aFormat); // create a copy 'cause we
                                                    // may modify it!
        ...
        /**
         * This method is required by the interface Iterator, but not really
        ...
        private LogPacket nextLogPacketFromStream() {
            ...
            if (available < this.minLogPacketSize) {
                // there's less then a full packet readable . . . this must be
                // filler!
                return null;
            }
                ...
                try {
                    ...
                    if (sep.isValid()) {
                        if (sep.changedLogFormat()) {
                            ...
                            this.minLogPacketSize = this.format
                                    .getMinPacketSize(this.parser);
                        }
                        // instead of rolling up the whole story again, just
                        // recurse . . .
    ...
    public Iterator<LogPacket> iterator() {
        return new ChunkIterator(this.dis, this.parser, this.format,
                this.minLogPacketSize);
    }
