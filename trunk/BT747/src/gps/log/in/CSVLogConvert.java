 ...
 * The {@link GPSRecord} is then sent to the {@link GPSFile} class object
...
public final class CSVLogConvert implements GPSLogConvert {
    ...
    public final int parseFile(final GPSFile gpsFile) {
        ...
        while (nextAddrToRead < fileSize) {
            ...
            do {
                ...
                if (continueInBuffer) {
                    StringBuffer s = new StringBuffer(eolPos - offsetInBuffer + 1);
                    ...
                    if (s.length() != 0) {
                        if (firstline) {
                            ...
                            activeFileFields = 0;
                            for (int i = 0;
                                 Fields.hasMoreTokens() && (i < C_MAX_RECORDS);
                                 i++) {
                            ...
                            while (Fields.hasMoreElements()
                                ...
                                if (field.length() != 0) {
                                    ...
                                    case C_LOGTIME:
                                        gpsRec.logPeriod = (int) (Convert.toFloat(field)*10);
                                        break;
                                    case C_LOGDIST:
                                        gpsRec.logDistance = (int) (Convert.toFloat(field)*10);
                                        break;
                                    case C_LOGSPD:
                                        gpsRec.logSpeed = (int)(Convert.toFloat(field)*10);
                                        break;
                                    case FMT_NS:
                                    ...
                                    {
                                        StringTokenizer n =
                                            new StringTokenizer(field.trim()," ");
                                        gpsRec.distance = 
                                            Convert.toDouble(n.nextToken())/3.28083989501312;
                                    }
                                        curLogFormat|=(1 << BT747Constants.FMT_DISTANCE_IDX);
                                        break;
                                ...
                                // Next condition should be handled by stopping
    ...
    private final void updateLogFormat(
            final GPSFile gpsFile,
