...
/**
 * A <code>RadioButton</code> component lets a user select exactly one
 * choice from a list. An example of where radio buttons are useful is to
 * select a logging level. There are a known set of choices and the level is
 * set to one of them.
 * <p>
 * Traditionally radio buttons are displayed next to each other in a box. This
 * component forgoes that model to save screen space. Instead the selected
 * option is displayed. If the user pressed a button while the component is
 * highlighted it takes them to a screen, similar to a menu, where the user
...
public class RadioButton extends Component {
    /**
     * The label that appears above the radio button. For example it might say
     * "Log Levels". If this is <code>null</code> then no label will appear.
     */
    private Label label;
    ...
    /**
     * This component appears as a <code>TextBox</code>. Only editing is
     * done differently. This component takes the user to another screen based
     * on <code>Menu</code>.
     */
    ...
    /**
     * @param label
     *                is the text that appears above the radio button. If
    ...
    public String getString(final int elementNum) {
        final String value = (String) choices.elementAt(elementNum);
        return value;
    }
     ...
     */
    public int append(final String choice) {
        if (choice == null) {
    ...
    /**
     * Inserts an element into the list of choices just prior to the element
     * specified.
     * 
     * @param elementNum
     *                is the index of the element where insertion is to occur.
     * @param choice
     *                is the string to be inserted.
     */
    public void insert(final int elementNum, final String choice) {
        if (choice == null) {
     ...
     * 
     * @param elementNum
     *                is the index of the element to be deleted.
     */
    ...
    /**
     * Sets the element referenced by <code>elementNum</code>, replacing
     ...
     */
    public void set(final int elementNum, final String choice) {
        if (choice == null) {
     ...
     */
    public void setSelectedIndex(final int elementNum) {
        if ((elementNum < 0) || (elementNum >= choices.size())) {
            throw new IndexOutOfBoundsException(
                    "elementNum not a possible choice");
        }
    ...
    protected int[] getPreferredComponentSize(final Theme theme,
        ...
        final int[] dimensions = box.getPreferredSize(theme, viewportWidth,
                viewportHeight);
     ...
     * When the user selects a new option the original screen returns and is
     * updated with the new value.
     */
    protected void select() {
        // Create a menu for selecting the choice.
        final Menu list = new Menu(getLabel(), getScreen());

        final Enumeration e = choices.elements();
        int index = 0;

        while (e.hasMoreElements()) {
            final String name = (String) e.nextElement();

            final RadioItem item = new RadioItem(name, index);
        ...
        final int highlight = getSelectedIndex();
    ...
    /**
     * Objects are used by the input screen to show the list of choices and
     * set the value of the one the user selected.
     */
    private final class RadioItem implements MenuItem {
         ...
         * 
         * @param choice
         *                is the name for this menu item.
         * @param index
         *                is the element number in <code>choices</code> this
         *                menu item represents.
         */
        public RadioItem(final String choice, final int index) {
            text = choice;
            this.index = index;
        }
        ...
        /**
         * Called when the user selects this radio item. It sets the radio
         * button component's selected value to this item.
         */
        public void onSelection() {
            ...
            final DeviceScreen screen = getScreen();
            screen.show();
            screen.repaint();
        }
    }
