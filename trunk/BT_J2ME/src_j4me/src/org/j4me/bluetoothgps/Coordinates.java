 ...
 * expressed in degrees using floating point values. The degrees are in
 * decimal values (rather than minutes/seconds). The coordinates are given
 * using the WGS84 datum.
 * <p>
 * This class also provides convenience methods for converting between a
 * string coordinate representation and the <code>double</code>
 * representation used in this class.
 */
public class Coordinates {
    /**
     * This is the earth's mean radius in meters. Using the mean gives the
     * most accurate results for distances measured with any bearing.
     * <p>
     ...
     * ellipsoid. <code>Float.Nan</code> can be used to indicate that
     ...
     * values indicate northern latitude and negative values southern
     * latitude.
     */
     ...
     * values indicate eastern longitude and negative values western
     * longitude.
     */
     ...
     * specified. The latitude and longitude parameters are expressed in
     * degrees using floating point values. The degrees are in decimal values
     * (rather than minutes/seconds).
     * <p>
     * The coordinate values always apply to the WGS84 datum.
     * <p>
     * The <code>Float.NaN</code> value can be used for altitude to indicate
     * that altitude is not known.
     * 
     * @param latitude -
     *                the latitude of the location. Valid range: [-90.0,
     *                90.0]. Positive values indicate northern latitude and
     *                negative values southern latitude.
     * @param longitude -
     *                the longitude of the location. Valid range: [-180.0,
     *                180.0). Positive values indicate eastern longitude and
     *                negative values western longitude.
     * @param altitude -
     *                the altitude of the location in meters, defined as
     *                height above WGS84 ellipsoid. <code>Float.Nan</code>
     *                can be used to indicate that altitude is not known.
     ...
     * to mean height above the WGS84 reference ellipsoid. 0.0 means a
     * location at the ellipsoid surface, negative values mean the location is
     * below the ellipsoid surface, <code>Float.Nan</code> that no altitude
     * is not available.
     * 
     ...
     *                at the ellipsoid surface, negative values mean the
     *                location is below the ellipsoid surface,
     *                <code>Float.Nan</code> that no altitude is not
     *                available
     * @see #getAltitude()
     */
     ...
     * double expressing the latitude in degrees in the WGS84 datum.
     * 
     * @param latitude -
    ...
    /**
     * Calculates the azimuth between the two points according to the
     * ellipsoid model of WGS84. The azimuth is relative to true north. The
     * Coordinates object on which this method is called is considered the
     * origin for the calculation and the Coordinates object passed as a
     * parameter is the destination which the azimuth is calculated to. When
     * the origin is the North pole and the destination is not the North pole,
     * this method returns 180.0. When the origin is the South pole and the
     * destination is not the South pole, this method returns 0.0. If the
     * origin is equal to the destination, this method returns 0.0. The
     * implementation shall calculate the result as exactly as it can.
     * However, it is required that the result is within 1 degree of the
     * correct result.
     * 
     * @param to -
     *                the <code>Coordinates</code> of the destination
     * @return the azimuth to the destination in degrees. Result is within the
     *         range [0.0 ,360.0).
     * @throws java.lang.NullPointerException -
     *                 if the parameter is <code>null</code>
     */
    public float azimuthTo(final Coordinates to) {
        if (to == null) {
        ...
        final double lon2 = Math.toRadians(to.longitude);
        ...
        final double c1 = Math.sin(deltaLon) * cosLat2;
        final double c2 = Math.cos(lat1) * Math.sin(lat2) - Math.sin(lat1)
                * cosLat2 * Math.cos(deltaLon);
        final double courseInRadians = MathFunc.atan2(c1, c2);
    ...
    /**
     * Calculates the geodetic distance between the two points according to
     * the ellipsoid model of WGS84. Altitude is neglected from calculations.
     * <p>
     * The implementation shall calculate this as exactly as it can. However,
     * it is required that the result is within 0.35% of the correct result.
     * 
     * @param to -
     *                the <code>Coordinates</code> of the destination
     ...
     *                 if the parameter is <code>null</code>
     */
    public float distance(final Coordinates to) {
        if (to == null) {
            throw new IllegalArgumentException(
        ...
        final double d1 = Math.sin((lat1 - lat2) / 2.0);
        final double d2 = d1 * d1;
        final double d3 = Math.cos(lat1) * Math.cos(lat2);
        final double d4 = Math.sin((lon1 - lon2) / 2.0);
        final double d5 = d4 * d4;
        final double d6 = d2 + d3 * d5;
        final double distanceInRadians = 2.0 * MathFunc.asin(Math.sqrt(d6));

        final double distance = Coordinates.METERS_PER_RADIAN
                * distanceInRadians;
     ...
     * 
     * @param other
     *                is another <code>Coordinates</code> object.
    ...
    public boolean equals(final Object other) {
        ...
        final Coordinates c = (Coordinates) other;
        ...
        if ((Float.isNaN(altitude) == true)
                && (Float.isNaN(c.altitude) == false)) {
            return false;
        }
        if ((Float.isNaN(altitude) == false)
                && (Float.isNaN(c.altitude) == true)) {
            return false;
        }
     ...
     * 
     * @return A string such as "79.32°N 169.8°W 25.7m" where the words are
     *         the latitude, longitude, and altitude (in meters).
     */
