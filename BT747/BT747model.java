    ...
    public int replyLogNmea(final String[] p_nmea) {
        if(p_nmea.length>2) {
            ...
            case BT747_dev.PMTK_LOG_REQ_DATA:
                ...
                StringBuffer s=new StringBuffer(BT747Settings.Hex2Int(p_nmea[3])*2);
            s.setLength(0);
            for (int i = BT747Settings.Hex2Int(p_nmea[3])*2; i > 0; i--) {
                s.append('F');
            }
            m_GPSrxtx.sendPacket("PMTK"+BT747_dev.PMTK_CMD_LOG_STR
                    +","+BT747_dev.PMTK_LOG_RESP_DATA
                    +","+p_nmea[2] // Address
                    +","+s
            );
            break;
