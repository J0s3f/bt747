...
//        ; 
    ...
    private static final int C_MAX_DISTANCE_IDX = C_minDist_IDX + C_minDist_SIZE;
    private static final int C_MAX_DISTANCE_SIZE = 8;
    private static final int C_minSpeed_IDX = C_MAX_DISTANCE_IDX + C_MAX_DISTANCE_SIZE;
...
//                bt747.sys.Vm.debug("loading config file "+CONFIG_FILE_NAME);
                ...
                try {
                    ...
                    if (readLength >= 100) {
                        ...
                        preferencesFile.readBytes(
                                appSettingsArray, 0, readLength);
...
//                Vm.debug("Exception new log delete");
            ...
            try {
                ...
                preferencesFile.writeBytes(
                        Settings.getAppSettings().getBytes(),
                        0,
...
//        setBaudRate(115200);
    ...
    private final void setOpt(
        ...
        + Settings.getAppSettings().substring((src.length() < (size - 1))
                ? (idx + src.length()) : (idx + size)))
        ;
        if (eventType != 0) {
            postEvent(eventType);
        }
    ...
    private final void setBooleanOpt(
            final int eventType,
            final boolean value,
            final int idx,
            final int size) {
    ...
    private final void setFloatOpt(
            final int eventType,
            final float src,
            final int idx,
            final int size) {
        setOpt(eventType,
                Convert.unsigned2hex(Convert.toIntBitwise(src), size),
                idx, size);
    }
    ...
    private final void setStringOpt(
            final int eventType,
            final String src,
            final int idx,
            final int size) {
        ...
        + ((src.length() < size) ? "\0" : "")
        + ((src.length() < (size - 1))
                ? new String(new byte[size - src.length() - 1]) : "")
        + ((Settings.getAppSettings().length() > idx + size)
               ?
                Settings.getAppSettings().substring(idx + size,
                                      Settings.getAppSettings().length())
                : "")
                )
        ;
	...
	/**
	 * @param baudRate The Baud rate to set as a default.
	 */
    ...
    public final int getChunkSize() {
        ...
        int chunkSize = getIntOpt(C_CHUNKSIZE_IDX, C_CHUNKSIZE_SIZE)
                            & 0xFFFFFFFE;
        if (chunkSize < 16) {
            chunkSize = 0x200;
        }
   ...
   public final void setDownloadTimeOut(final int downloadTimeOut) {
       setIntOpt(0, downloadTimeOut,
               C_DOWNLOADTIMEOUT_IDX, C_DOWNLOADTIMEOUT_SIZE);
   }
   ...
   public final int getTimeOffsetHours() {
       int timeOffsetHours =
           getIntOpt(C_TIMEOFFSETHOURS_IDX, C_TIMEOFFSETHOURS_SIZE);
   ...
   public final void setTimeOffsetHours(final int timeOffsetHours) {
       setIntOpt(0, timeOffsetHours,
               C_TIMEOFFSETHOURS_IDX, C_TIMEOFFSETHOURS_SIZE);
   }
    ...
    public final String getColorInvalidTrack() {
        return getStringOpt(
                C_COLOR_INVALIDTRACK_IDX, C_COLOR_INVALIDTRACK_SIZE);
    }
     ...
     */
    protected final void setSolveMacLagProblem(
            final boolean solveMacLagProblem
            ) {
    ...
    protected final void setTraversableFocus(final boolean traversableFocus) {
        setBooleanOpt(0, traversableFocus,
                C_ISTRAVERSABLE_IDX, C_ISTRAVERSABLE_SIZE);
    }
    ...
    protected final void setLogFormatConditionSetting1(final int value) {
        setIntOpt(0, value,
                C_SETTING1_LOG_FORMAT_IDX, C_SETTING1_LOG_FORMAT_SIZE);
    }
    ...
    protected final void setRecordNbrInLogs(final boolean value) {
        setBooleanOpt(0, value,
                C_RECORDNBR_IN_LOGS_IDX, C_RECORDNBR_IN_LOGS_SIZE);
    }
