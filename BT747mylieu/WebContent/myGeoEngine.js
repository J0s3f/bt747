	...
	init:function()
	{
		...
		switch(this.myEngine)
		{
			...
			case "OSM":
				...
				var lonLat = new OpenLayers.LonLat(lon, lat).transform(new OpenLayers.Projection("EPSG:4326"), this.myMap.getProjectionObject());
...
// fun: will standard marker
	 ...
	 * @parameter string
	 */
	createIcon:function(anIconName, anIcon_X, anIcon_Y, 
	                            anShadowIconName, anShadowIcon_X, anShadowIcon_Y)
	{
		var anIcon;
	...
	createLatLong:function(aLatitude, aLongitude)
	{
		switch(this.myEngine)
		{
			...
			case "OSM":
				return new OpenLayers.LonLat(aLongitude, aLatitude).transform(new OpenLayers.Projection("EPSG:4326"), this.myMap.getProjectionObject());
				break;
	...
	addPolyLine:function(pointsArray)
	{
		switch(this.myEngine)
		{
			case "google":
				...
				this.myMap.setCenter(myPolyLine.getBounds().getCenter(), this.myMap.getBoundsZoomLevel(myPolyLine.getBounds()));
				break;
			...
			case "OSM":
				...
				{
					point = new OpenLayers.Geometry.Point(pointsArray[i].lon, pointsArray[i].lat);
					points.push(point);
				}
				...
				// center and zoom to the bounding box
				this.myMap.setCenter(geometry.getBounds().getCenterLonLat(), this.myMap.zoomToExtent(geometry.getBounds()));
				break;
