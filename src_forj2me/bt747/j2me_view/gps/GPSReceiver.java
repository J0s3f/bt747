    ...
    implements Runnable, CThread.ThreadListener
{
    ...
    */
    private GPSReceiver(GPSDevice gpsDev)
    {
        try {
            ...
            this.acquireThread  = new CThread("GPS", this);
    ...
    */
        ...
        throws SecurityException
    {
        ...
        try {
            ...
            /* GPS read loop */
            // Hnag here until one of the following occurs:
            // - We get a GPS error
    ...
    */
    private boolean _parseNMEA0183(String data)
    {
        ...
        if (dataUC.startsWith("$PG") ||    // <-- Garmin sentence
            dataUC.startsWith("$GPGSV")) { // <-- Satellite info sentence
        ...
        /* proper NMEA-0183 sentence */
        if (!dataUC.startsWith("$GP")) {
        ...
        if (!this._hasValidChecksum(data)) {
            Log.warn(LOG_NAME, "Failed Checksum");
            return false;
        }

        /* parse record */
        String fld[] = StringTools.parseString(dataUC, ',');
