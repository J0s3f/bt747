...
//***  WabaSoft, Inc.                                              ***
//********************************************************************                              
package net.sourceforge.bt747.gps.connection;

import net.sourceforge.bt747.gps.Semaphore;
...
public class GPSrxtx {
    ...
    public void sendPacket(final String p_Packet) {
        if (isConnected()) {
            ...
            if (GPS_DEBUG) {
                net.sourceforge.bt747.bt747.sys.Vm.debug(">" + rec.toString());
            }
    ...
    final static int DPL700_MODE = 1;
    ...
    public void sendCmdAndGetDPL700Response(int cmd, int buffer_size) {
        if (isConnected()) {
            ...
            DPL700_buffer_idx = 0;
            if (GPS_DEBUG) {
                net.sourceforge.bt747.bt747.sys.Vm.debug(">0x" + Convert.unsigned2hex(cmd, 8)
                        + "000000");
            }
    ...
    public void sendCmdAndGetDPL700Response(String cmd, int buffer_size) {
        if (isConnected()) {
            ...
            DPL700_buffer_idx = 0;
            if (GPS_DEBUG) {
                net.sourceforge.bt747.bt747.sys.Vm.debug(">" + cmd);
            }
            rec.setLength(0);
            rec.append(cmd);
            rec.append("\0");
    ...
    public String[] getResponse() {
        ...
        if (gpsPort.debugActive()) {
            ...
            gpsPort.writeDebug("\r\nR:"
                    + Convert.toString(net.sourceforge.bt747.bt747.sys.Vm.getTimeStamp()) + ":");
        }
