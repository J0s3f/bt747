...
import net.sourceforge.bt747.gps.connection.GPSrxtx;
...
import net.sourceforge.bt747.bt747.generic.Generic;
import net.sourceforge.bt747.bt747.io.File;
import net.sourceforge.bt747.bt747.sys.Convert;
import net.sourceforge.bt747.bt747.sys.Thread;
import net.sourceforge.bt747.bt747.sys.Vm;
...
public class GPSstate implements Thread {
    ...
    private void openNewLog(final String fileName, final int card) {
        try {
            ...
            logFile = new File(fileName, net.sourceforge.bt747.bt747.io.File.DONT_OPEN, card);
            logFileCard = card;
            if (logFile.exists()) {
                logFile.delete();
            }

            logFile = new File(fileName, net.sourceforge.bt747.bt747.io.File.CREATE, card);
            // lastError 10530 = Read only
            logFileCard = card;
            logFile.close();
            logFile = new File(fileName, net.sourceforge.bt747.bt747.io.File.READ_WRITE, card);
            logFileCard = card;
