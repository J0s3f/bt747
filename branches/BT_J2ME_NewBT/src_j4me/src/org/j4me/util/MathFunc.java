...
/**
 * Implements the methods which are in the standard J2SE's <code>Math</code>
 * class, but are not in in J2ME's.
 * <p>
...
public final class MathFunc {
     ...
     * <code>Math.PI</code>. Special case:
     * <ul>
     * <li>If the argument is <code>NaN</code> or its absolute value is
     * greater than 1, then the result is <code>NaN</code>.
     * </ul>
     * 
     * @param a -
    ...
    public static double acos(final double a) {
        ...
        final double arcCosine = MathFunc.atan2(Math.sqrt(1 - aSquared), a);
        return arcCosine;
    }

    /**
     * Returns the arc sine of an angle, in the range of
     * <code>-Math.PI/2</code> through <code>Math.PI/2</code>. Special
     * cases:
     * <ul>
     * <li>If the argument is <code>NaN</code> or its absolute value is
     * greater than 1, then the result is <code>NaN</code>.
    ...
    public static double asin(final double a) {
        ...
        final double aSquared = a * a;
        final double arcSine = MathFunc.atan2(a, Math.sqrt(1 - aSquared));
        return arcSine;
    }
     ...
     * cases:
     * <ul>
     * <li>If the argument is <code>NaN</code>, then the result is
     * <code>NaN</code>.
     * <li>If the argument is zero, then the result is a zero with the same
     * sign as the argument.
     * </ul>
    ...
    public static double atan(double a) {
        ...
        for (; a > MathFunc.PIover12; a *= t) {
            i++;
            t = a + MathFunc.ATAN_CONSTANT;
            t = 1.0 / t;
            a *= MathFunc.ATAN_CONSTANT;
            a--;
        }
        ...
        for (; i > 0; i--) {
            arcTangent += MathFunc.PIover6;
        }

        if (greaterThanOne) {
            arcTangent = MathFunc.PIover2 - arcTangent;
        }
    ...
    /**
     * Converts rectangular coordinates (x, y) to polar (r, <i>theta</i>).
     * This method computes the phase <i>theta</i> by computing an arc
     * tangent of y/x in the range of <i>-pi</i> to <i>pi</i>. Special
     * cases:
     * <ul>
     * <li>If either argument is <code>NaN</code>, then the result is
     * <code>NaN</code>.
     ...
     * argument is negative infinity, then the result is the
     ...
     * is negative infinity, then the result is the double value closest to 3*<i>pi</i>/4.
     * <li>If the first argument is negative infinity and the second argument
     * is positive infinity, then the result is the double value closest to -<i>pi</i>/4.
     * <li>If both arguments are negative infinity, then the result is the
     * double value closest to -3*<i>pi</i>/4.
     * </ul>
     * <p>
    ...
    public static double atan2(final double y, final double x) {
        ...
        if (Double.isNaN(y) || Double.isNaN(x)) {
            ...
            if (y > 0.0) // Positive infinity
            {
                if (Double.isInfinite(x)) {
                    if (x > 0.0) {
                        return MathFunc.PIover4;
                    } else {
                        return 3.0 * MathFunc.PIover4;
                    }
                } else if (x != 0.0) {
                    return MathFunc.PIover2;
                }
        ...
        // Implementation a simple version ported from a PASCAL
        // implementation:
        ...
        if (Math.abs(x) > Math.abs(y)) {
            arcTangent = MathFunc.atan(y / x);
        } else {
            arcTangent = MathFunc.atan(x / y); // -PI/4 <= a <= PI/4

            if (arcTangent < 0) {
                arcTangent = -MathFunc.PIover2 - arcTangent; // a is
                                                                // negative,
                                                                // so we're
                                                                // adding
            } else {
     ...
     * rounded to an integer by adding 1/2, taking the floor of the result,
     * and casting the result to type <code>int</code>. In other words, the
     ...
     * rounded to an integer by adding 1/2, taking the floor of the result,
     * and casting the result to type <code>long</code>. In other words,
