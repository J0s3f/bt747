 ...
 * bit integer than those that have defined constants in this class are
 * reserved and MUST not be set by implementations (i.e. these bits must be
 * 0).
 * <p>
 ...
 * and the <code>getQualifiedCoordinates</code> method must return their
 ...
 * <code>LocationListener</code> may return invalid <code>Location</code>
 ...
 * it only returns objects where the parameters have values set as described
 * for their semantics in this class.
 */
public interface Location {
    /**
     * Returns the coordinates of this location and their accuracy.
     * 
     * @return a <code>QualifiedCoordinates</code> object. If the
     *         coordinates are not known, returns <code>null</code>.
     */
     ...
     *         <code>Float.NaN</code> if the speed is not known
     */
     ...
     *         north or <code>Float.NaN</code> if the course is not known
     */
     ...
     * clock in the terminal in milliseconds using the same clock and same
     * time representation as <code>System.currentTimeMillis()</code>.
     * 
     ...
     * coordinates or an invalid one where all the data, especially the
     * latitude and longitude coordinates, may not be present.
     * 
     * A valid <code>Location</code> object contains valid coordinates
     * whereas an invalid <code>Location</code> object may not contain valid
     * coordinates but may contain other information via the
     * <code>getExtraInfo()</code> method to provide information on why it
     * was not possible to provide a valid <code>Location</code> object.
     * 
     * @return a boolean value with true indicating that this
     *         <code>Location</code> instance is valid and false indicating
     *         an invalid <code>Location</code> instance
     */
