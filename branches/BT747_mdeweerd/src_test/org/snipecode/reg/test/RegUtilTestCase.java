...
public class RegUtilTestCase extends TestCase
{
	...
	{
    	String regPath="SOFTWARE\\SnipeTest";
    	int handle;       
        // Create a key
        int result[] = RegUtil.RegCreateKeyEx(RegUtil.HKEY_CURRENT_USER, regPath);
        
        //close the handle
        RegUtil.RegCloseKey(result[RegUtil.NATIVE_HANDLE]);

        //open a new handle with all access
        handle = RegUtil.RegOpenKey(RegUtil.HKEY_CURRENT_USER, regPath,RegUtil.KEY_ALL_ACCESS)[RegUtil.NATIVE_HANDLE];
        ...
        assertEquals("TestValue", new String(val).toString().trim());
        // Close the handle
        //delete the value
        RegUtil.RegDeleteKey(RegUtil.HKEY_CURRENT_USER, regPath);
    }
