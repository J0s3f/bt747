...
public final class NaviLinkHandler {
    ...
    // NAVILINK Protocol
    //
    // This page is based on the Navilink protocol specification of Jun 30,
    // 2005 provided by Locosys as PDF. Things found to be different in the
    // real protocol implementation have been fixed in the specification
    // below.
    // 1. General Description
    //
    // This document describes the NAVILINK Interface, which is used to
    // communicate with a NAViGPS device. The NAVILINK Interface supports
    // bi-directional transfer of data such as waypoints, routes, track data.
    // 2. Physical Layer
    //
    // The physical protocol is based on USB. The electrical characteristics
    // are full duplex, serial data, 115200 baud, 8 data bits, no parity bits
    // and 1 stop bit.
    ...
    // 0xA0, 0xA2 Two-bytes (15-bits) Up to 215-1 Two-bytes (15-bits) 0xB0,
    // 0xB3
    //
    // Bytes in a packet are transmitted low order byte first, followed by the
    // high byte (little-endian order).
    //
    // *
    // Packet Length
    // o
    ...
    // Payload
    // o
    // The first byte of payload data is always the Packet ID (described in
    // section 4).
    // *
    // Checksum
    // o
    // The checksum is 15-bit checksum of the bytes in the payload data. The
    // following pseudo code defines the algorithm used. Let Packet to be the
    // array of bytes to be sent by the transport. Let msgLen be the number of
    // bytes in the Packet array to be transmitted.
    //
    // Index = first
    // checkSum = 0
    // while index < msgLen
     ...
     */
    private final void sendNaviPacket(final byte cmd, final byte[] payload,
            final int len) {
        final byte[] lenbytes = new byte[2];
        final byte[] checksumbytes = new byte[2];
        int length;
        int checksum = cmd;
        ...
        sendPacket(NaviLinkHandler.PACKET_START,
                NaviLinkHandler.PACKET_START.length);
        sendPacket(lenbytes, 2);
        if (payload != null) {
            sendPacket(payload, len);
        }
        sendPacket(checksumbytes, 2);
        sendPacket(NaviLinkHandler.PACKET_END,
                NaviLinkHandler.PACKET_START.length);
    }
    ...
    // type the sending direction is given (H->D, H<-D or H<->D), where H is
    // the
    // Host computer and D the NAViGPS device.
    //
    // For communicating with the device, the NAVILINK Mode needs to be
    // enabled.
    // In NAViGPS side, you can select function NAVILINK to set NAViGPS in
    // link
    // mode. You may exit from link mode by pressing any key or send a
    // PID_QUITE
    ...
    // Packet PID_ACK is received if the connection is OK. After the
    // connection
    ...
    // should be uploaded first. You may first back up all waypoints and
    // routes
    // then download new waypoints and new routes to guarantee data
    // consistency.
    // 
    //
     ...
     * 
     * Expect to receive a PID_DATA packet with payload data in T_INFORMATION
     * type
     */
     ...
     * 
     * The device will return a PID_DATA packet holding the firmware version
     * as a string.
     */
    ...
    public final void reqNavilinkSendData(final byte[] bytes, final int length) {
    ...
    // Byte 0: 0x28
    // *
    // Byte 1..4: the first waypoint to query by this packet, 0 based,
    // waypoints are sorted by name
    // *
    // Byte 5..6: number of waypoints to query (1..32)
    // *
    // Byte 7: 0x01
    //
    // Expect to receive PID_DATA packet with payload in T_WAYPOINTS, receive
    // PID_NAK if no waypoints read.
    //
    // For example, to read the first waypoint by sending: [A0 A2 08 00 28 00
    // 00 00 00 01 00 01 2A 00 B0 B3]
    //
    // To read the second and third waypoints by sending: [A0 A2 08 00 28 01
    // 00 00 00 02 00 01 2C 00 B0 B3]
    // PID_QRY_ROUTE
    //
    ...
    //
    // For example, to query the first route by sending: [A0 A2 08 00 24 00 00
    // 00 00 00 00 01 25 00 B0 B3]
    // PID_DEL_WAYPOINT
    //
    ...
    //
    // Expect to receive PID_ACK if successful, else PID_NAK is received. The
    // waypoint used by any routes cannot be deleted.
    //
    // For example, to delete a waypoint with ID 01 by sending: [A0 A2 05 00
    // 36 00 00 01 00 37 00 B0 B3]
    ...
    //
    // Expect to receive PID_ACK if successful else PID_NAK is received. You
    // have to delete all routes before deleting all waypoints.
    //
    // For example, to delete all routes by sending: [A0 A2 05 00 37 00 00 f0
    // 00 27 01 B0 B3]
    // PID_DEL_ROUTE
    //
    ...
    //
    // For example, to delete a route with ID 01 by sending: [A0 A2 05 00 34
    // 00 00 01 00 35 00 B0 B3]
    // PID_DEL_ALL_ROUTE
    //
    ...
    // Expect to receive PID_ACK if successful else PID_NAK is received.
    //
    // For example, to delete all routes by sending [A0 A2 05 00 35 00 00 f0
    // 00 25 01 B0 B3]
    // PID_ADD_A_ROUTE
    //
    // Packet to add a route to NAViGPS
    ...
    // Byte 0: 0x11
    // *
    // Byte 1..4: track buffer address, typical value is 0x400e0000, value can
    // be found in T_INFORMATION.pTrackBuf
    // *
    // Byte 5..6: 0x0000
    // *
    // Byte 7 : always 0x00
    ...
    // in 4 seconds if successful. Send PID_ACK if data are received
    // correctly.
    //
    // For example, to read a track of 2561) points, 2 packets of
    // PID_READ_TRACKPOINTS are needed. The start address and byte length are
    // as
    // follows:
    ...
    // Byte 0: 0x16
    // *
    // Byte 1..4: start address(track buffer address + offset) typical value
    // is 0x400e0000
    // *
    ...
    // Byte 7: always 0x00
    //
    // After sending PID_WRITE_TRACKPOINTS packet, wait a short while (about
    // 10ms seems to be enough) then send a PID_DATA packet with trackpoint
    // data. Then you can expect to receive PID_CMD_OK within 4 seconds if
    // successful. The maximum track points in one PID_DATA packet is 127. For
    // example, to write a track of 520 points, 5 packets of
    // PID_WRITE_TRACKPOINTS are needed.
    // PID_CMD_OK
    //
    ...
    // subroutes. Its length is variable and depends on the number of
    // subroutes
    ...
    // types) consists 1 to maximum 14 waypoint IDs. A null waypoint ID
    // (0xffff)
    // must be appended after the last waypoint ID(0..999) in the last
    // subroute
    // of a route.
    //
    // If you happen to send a route with exactly 14 waypoints (or 28...) to
    // the
    // device, you may be better off adding another subroute, filled with
    // 0xffff
    // as waypoints. What is used by the NaviGPS in the second subroute added
    // seems rather random to me. But the NaviGPS adds this second subroute
    // too
    ...
    // same UTM zone.
    //
    // Note: The UTM zone information seems not to be related to the
    // trackpoint
