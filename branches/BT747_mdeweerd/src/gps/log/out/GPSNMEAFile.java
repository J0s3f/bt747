...
public final class GPSNMEAFile extends GPSFile {
    ...
    private void writeRMC(final GPSRecord s, final String timeStr) {
        // eg4.
    ...
    private void writeGGA(final GPSRecord s, final String timeStr) {
        if ((m_NMEAout & (1 << BT747Constants.NMEA_SEN_GGA_IDX)) != 0) {
            ...
            if ((activeFields.height != 0)) {
                ...
                if ((activeFields.latitude != 0)
                    ...
                    rec.append(",M,");
                    rec.append(Convert.toString(Conv.wgs84Separation(
                            s.latitude, s.longitude), 1));
                    rec.append(",M,");
                } else {
                    rec.append(",M,0,M,");
                }
    ...
    private void writeGSV(final GPSRecord s, final String timeStr) {
        // $GPGSV
    ...
    private void writeZDA(final GPSRecord s, final String timeStr) {
        if ((activeFields.utc != 0)
                && ((m_NMEAout & (1 << BT747Constants.NMEA_SEN_ZDA_IDX)) != 0)) {
            /*
     ...
     private void writeGSA(final GPSRecord s, final String timeStr) {
        ...
        if (activeFields.sid != null) {
            ...
            if ((s.sid != null)
                    && (m_NMEAout & (1 << BT747Constants.NMEA_SEN_GSA_IDX)) != 0) {
                rec.setLength(0);
                rec.append("GPGSA,A,");
                if ((activeFields.valid != 0)) {
                    if (s.valid == 1) {
