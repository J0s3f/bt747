...
public class GPSKMLFile extends GPSFile {
	...
	private final void startTrack(final String name) {
		...
		text = "<Placemark>" + "<name>" + JavaLibBridge.convertHTMLtoUTF8(name)
				+ "</name>" + "\r\n" + "    <Style>\r\n"
				+ "      <LineStyle>\r\n" + "        <color>ff" + color
				+ "</color>\r\n" + "        <width>3.0</width>\r\n"
				+ "      </LineStyle>\r\n" + "    </Style>\r\n";
	...
	public final void writeRecord(final GPSRecord r) {
		...
		if (ptFilters[currentFilter].doFilter(r)) {
			String dateString = "";
			if (r.hasUtc()) {
				dateString = t.getYear() + "-" + (t.getMonth() < 10 ? "0" : "")
			...
			if (isWayType
				...
				if (r.hasPosition() && selectedFileFields.hasPosition()) {
					...
					rec.append(JavaLibBridge.toString(r.getLongitude(),
							posDigits));
					rec.append(",");
					rec.append(JavaLibBridge.toString(r.getLatitude(),
							posDigits));
