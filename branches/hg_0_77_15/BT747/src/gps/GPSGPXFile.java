  ...
  public void writeRecord(GPSRecord s) {
      ...
      if(activeFields!=null) {
          if(!m_Filter.doFilter(s) ||
            // HGHGHG: waypoints only for Button events 
                (m_isWayType && !((activeFields.rcr!=0) &&  
                                  (s.rcr&BT747_dev.RCR_BUTTON_MASK)!=0))) {
              // filtered
...
//                  <fix> fixType </fix> [0..1] ?
              ...
              if((activeFields.rcr!=0)) {
                  ...
                  rec+=style;
                  rec+="</type>\r\n";
                  type=style;   // for comment
              }
              
              //                    <sat> xsd:nonNegativeInteger </sat> [0..1] ?
              //                    <hdop> xsd:decimal </hdop> [0..1] ?
              if((activeFields.hdop!=0)) {
                  hdop=Convert.toString(s.hdop/100.0,2); 
                  rec+="<hdop>";
                  rec+=hdop; 
                  rec+="</hdop>\r\n";
              }
              ...
              if((activeFields.nsat!=0)) {
                    nsat+=Convert.toString(s.nsat/256); 
                    nsat+="(";
                    nsat+=Convert.toString(s.nsat%256); 
                    nsat+=")";
                rec+="<nsat>";
